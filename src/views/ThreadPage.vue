...
const thread = threadStore.thread(id)
...
<template>
	<div>
		...
		<div class="col-large push-top">
			...
			<h1>{{ thread.title }}</h1>
			<router-link 
			:to="{name: 'editThread', params: { id: thread.id }}" class="btn-green btn-small">
				Edit
			</router-link>
			<p>
				By <a href="#" class="link-unstyled">{{thread.author.name}}</a>, <AppDate :timeStamp="thread.publishedAt"/>.
				<span style="float: right; margin-top: 2px" class="hide-mobile text-faded text-small"
					>{{ thread.repliesCount }} replies by {{ thread.contr }} contributors</span
				>
			</p>
			<PostList :posts="postStore.filterByThreadId(id)" />
			<PostEdit @update-posts="addPost"/>
		</div>
