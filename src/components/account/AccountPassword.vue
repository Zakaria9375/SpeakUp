...
import { changePassword } from '@/validations/schemas.js'
...
const savePassword = handleSubmit(async (values) => {
	...
	if (valid) {
		try {
		updating.value = true
    const result = await account.updatePassword(values.newPassword, values.oldPassword);
    console.log( 'PU: sccuess', result);
		close();
		updating.value = false
  } catch (error) {
    console.error( 'Err password update', error); 
  }
	}
})
</script>
...
<template>
	<div class="acc-card">
		...
		<div class="card-content">
			...
			<button type="button" @click="open" class="nobtn">Change password ></button>
		</div>
	</div>
	<PopUp class="del-main" v-if="refValue" @close="close">
